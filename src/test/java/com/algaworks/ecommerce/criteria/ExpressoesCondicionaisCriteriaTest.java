...
public class ExpressoesCondicionaisCriteriaTest extends EntityManagerTest {
    ...
    public void usarExpressaoIN03() {
        ...
        criteriaQuery.where(root.get("cliente").get("id").in(clientes));
...
//                        .otherwise(root.get(Pedido_.status))
	...
	@Test
	public void usarMaiorMenosComDatas() {
		...
		criteriaQuery
				.where(criteriaBuilder.greaterThanOrEqualTo(root.get("dataCriacao"), LocalDateTime.now().minusDays(3)));

		TypedQuery<Pedido> typedQuery = entityManager.createQuery(criteriaQuery);
		List<Pedido> lista = typedQuery.getResultList();
		Assertions.assertFalse(lista.isEmpty());

		lista.forEach(p -> System.out.println("ID: " + p.getId() + ", Data de criação: " + p.getDataCriacao()));
	}
	...
	public void usarMaiorMenor() {
		...
		criteriaQuery.where(criteriaBuilder.greaterThanOrEqualTo(root.get("preco"), new BigDecimal(799)),
	...
	public void usarIsNotNull() {
		...
		criteriaQuery.where(criteriaBuilder.isNotNull(root.get("foto")));
